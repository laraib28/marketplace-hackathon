...
const ProductPage = () => {
  ...
  const [product, setProduct] = useState<SellingProduct | null>(null);
  ...
  useEffect(() => {
    if (slug) {
      const fetchProduct = async () => {
        ...
        try {
          const products: SellingProduct[] = await client.fetch(query, { slug });
  ...
  // In your SellingProduct Component, ensure that the `handleAddToCart` is working properly.
const handleAddToCart = (product: SellingProduct) => {
  dispatch(
    addToCart({
